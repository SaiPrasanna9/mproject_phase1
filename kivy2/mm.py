...
class clockApp(App):
    ...
    def midn(self):
        if(self.root.ids.start_stop.text=='Stop'):
            self.i=self.i+1
            print("***"*10,self.i)


    def come1(self,audio):
        try:
    ...
    def come(self):
        try:
            ...
            for i in rem_list:
                some=some+i+" ---- "+rem_list[i]
            for i in (rem_list):
                self.root.add_widget(Label(text =i+" :: "+rem_list[i],color = (1,0,1,1)))
    ...
    def start_stop(self):
        self.tf=True
        self.event = Clock.schedule_interval(self.midn, 1./20.)
