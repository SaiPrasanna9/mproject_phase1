...
<RootFloatLayout>:
    ...
    Button:
        text: "Start"
        font_size:20
        background_normal :'greenstart.png'
        background_down:'greenstart.png'
        border:30,30,30,30
        size_hint:0.2,0.23
    ...
    Button:
        text: "Stop"
        font_size:20
        background_normal :'reds.png'
        #background_down:'reds.png'
        size_hint: 0.2,0.19
...
class RootFloatLayout(FloatLayout):
    ...
    def come(self):
        try:
            ...
            self.layout = GridLayout(cols=2,rows=leng+1,row_default_height= 20,row_force_default=True,padding=[0,0,0,0],spacing=[0,0],pos_hint= {"x":0.2, "y":-0.6})
            ...
            for i in (self.rem_list):
                ...
                self.layout.add_widget(Label(text =i +" :: "+self.rem_list[i],color = (1,0,1,1),font_size="10dp"))
                #self.active = CheckBox(active = False,color=(1,0,1,1),width="230")
                x = CheckBox(active = False,color=(1,0,1,1),width="200")
                self.cbs[x]=i

                self.layout.add_widget(x)
            #self.layout.add_widget(Button(text='Ok'))
            #self.layout.add_widget(Button(text="Ok",font_size=10,column_force_default=True,column_default_width=10,size_hint=(0.5,0.08),pos_hint={"x": 0.45, "y": 0.08},background_color='#e6005c'))    
