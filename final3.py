...
class RootFloatLayout(FloatLayout):
    ...
    def come(self):
        try:
            ...
            self.layout = GridLayout(cols=2,rows=leng,row_default_height= 20,row_force_default=True,padding=[0,0,0,0],spacing=[0,0],pos_hint= {"x":0.2, "y":-0.6})
            ...
            for i in (self.rem_list):
                ...
                self.layout.add_widget(Label(text =i +" :: "+self.rem_list[i],color = (0,0,0,1),font_size="10dp"))
                #self.active = CheckBox(active = False,color=(1,0,1,1),width="230")
                x = CheckBox(active = False,color=(0,0,0,1),width="200")
                self.cbs[x]=i
